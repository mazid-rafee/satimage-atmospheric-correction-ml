...
def train_and_evaluate(bands, combo_index, total_combos, encoder_name, encoder_weights, result_path, mode="binary"):
    ...
    print(f"\n--- Training UNet++ ({encoder_name}) on band combo {combo_index + 1}/{total_combos} | Mode: {mode} ---")

    train_dataset = CloudSegmentationDataset(taco_path, train_idx, bands, mode=mode)
    test_dataset = CloudSegmentationDataset(taco_path, test_idx, bands, mode=mode)
    ...
    for epoch in range(10):
        ...
        print(f"[{encoder_name}] Epoch {epoch+1} Avg Loss: {avg_loss:.4f}")

    model.eval(); total_iou = [0]*4 if mode == "multiclass" else 0
    ...
    with open(result_path, "a") as f:
        f.write(f"UNet++ | Encoder: {encoder_name} | Combo {combo_index+1}/{total_combos} | Bands: {bands} | Mode: {mode} | {metrics}\n")
    ...
    for encoder_cfg in encoder_configs:
        for idx, bands in enumerate(band_combinations):
            train_and_evaluate(
                bands,
                idx,
                len(band_combinations),
                encoder_name=encoder_cfg["name"],
                encoder_weights=encoder_cfg["weights"],
